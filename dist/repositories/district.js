...
const district_1 = __importDefault(require("../entities/district"));
const district_2 = __importDefault(require("../models/district"));
let DistrictRepository = class DistrictRepository {
    ...
    // }
    async findOne(uuid) {
        const province = await district_2.default.findOne({ uuid: uuid });
        return province ? new district_1.default(province) : null;
    }
    ...
    async findAll(query) {
        ...
        return district_2.default.find({ ...queryVal }, {}, options)
            .then(result => {
            return result.map(data => {
                return new district_2.default({
                    uuid: data.uuid,
                    name: data.name,
                    code: data.code,
                });
