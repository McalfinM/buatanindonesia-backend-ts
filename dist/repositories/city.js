...
const city_2 = __importDefault(require("../models/city"));
let CityRepository = class CityRepository {
    ...
    // }
    async findOne(uuid) {
        const city = await city_2.default.findOne({ uuid: uuid });
        return city ? new city_1.default(city) : null;
    }
    ...
    async findAll(query) {
        ...
        return city_2.default.find({ ...queryVal }, {}, options)
            .then(result => {
            return result.map(data => {
                return new city_1.default({
                    uuid: data.uuid,
                    name: data.name,
                    code: data.code,
