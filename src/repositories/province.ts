...
import { IProvinceRepository } from "./interfaces/province";
...
class ProvinceRepository implements IProvinceRepository {
    ...
    async findOne(uuid: string): Promise<ProvinceEntity | null> {
        const province = await ProvinceModel.findOne({ uuid: uuid })
        return province ? new ProvinceEntity(province) : null
    }
    ...
    async findAll(query: { [k: string]: any }): Promise<ProvinceEntity[]> {
